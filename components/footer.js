    ...
    <footer className={`fixed left-0 right-0 bottom-0 `}>
      <div className="absolute md:bottom-24 bottom-24 left-4 md:right-auto right-4 max-w-[345px] mx-auto">
        ...
        <div className="flex items-center border border-black/50">
          <div
            className="bg-[#EBFD00] md:p-3 md:h-9 md:w-9 h-8 w-7 p-2 cursor-pointer"
            onClick={() => {
              setAudio(!audio);
            }}
          >
            {audio ? (
              <Image src={Stop} alt="" className="object-contain" />
            ) : (
              <Image src={Play} alt="" className="object-contain" />
            )}
          </div>
      ...
      <div
        className={`border-t-[13px] border-[#EBFD00]  ${
          link == url ? 'block' : 'md:block hidden'
        } ${router.pathname == '/' && 'md:block hidden'}`}
      >
        <div>
          <Slider {...settings}>
              ...
              <div key={i} className="bg-[#F2F2F2]">
                ...
                ${link == item.url && 'text-[#F2F2F2]'}`}
                  >
                    ...
                    -{' '}
                    <span className="HelveticaNowText_Regular italic">
                      {' '}
                      {item.title2}
                    </span>
                  </Link>
                  <p
                    className={`text-xs leading-4 px-2 pt-[6px] pb-[6px] uppercase NeueHaasRoman h-full items-center inline-flex border-l border-[#707070] ${
